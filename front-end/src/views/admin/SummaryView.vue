  ...
  <main>
    <div class="flex flex-col">
      ...
      <div class="w-3/4 mx-auto pt-10 pb-28">
        <h1 class="text-center font-bold text-3xl py-4">
          Peminjaman Barang Summary
        </h1>
        <table
          id="pdf-printable"
          ...
          <tbody>
            <tr v-for="(i, index) in items" :key="i._id">
              ...
              <td
                class="px-4 py-1 border border-slate-300 font-bold text-center"
              >
                {{ moment(i.borrow_date).format("YYYY-MM-DD") }}
              </td>
              <td
                class="px-4 py-1 border border-slate-300 font-bold text-center"
              >
                {{ moment(i.return_date).format("YYYY-MM-DD") }}
              </td>
              <td class="px-4 py-1 border border-slate-300 font-bold">
                {{ i.borrower_name }}
              </td>
        ...
        <button
          @click="exportToPDF()"
          class="mt-5 py-2 px-3 bg-green-500 rounded-xl text-white font-semibold shadow-xl hover:bg-green-700"
        >
          Cetak PDF
        </button>
      </div>
